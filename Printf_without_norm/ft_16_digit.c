...
/*   Updated: 2021/01/26 20:49:13 by tchariss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char 		p[17]; // массив , с памятью 21 -> /// char 		p[21]
/*
Глобальные переменные используют 21 байт (1%) динамической памяти, 
оставляя 2 027 байт для локальных переменных. 
Максимальное значение составляет 2 048 байт.
*/
	...
	i = len;
	//p = (char *)malloc(sizeof(char) * (len) + 1); // +1 для '\0'
	while (digit >= 16)
	{
		if (c == 'x' || c == 'p')
